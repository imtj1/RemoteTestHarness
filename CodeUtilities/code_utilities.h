...
namespace Utilities
{
	...
	inline void preface(const std::string& msg = "", bool doReturn = true, std::ostream& out = std::cout,
	                    const std::string& prefix = "  ")
	{
		if (doReturn) {
			logger_provider::get_logger().log_file_content("\n");
			logger_provider::get_logger().log_file_content("\n");
			logger_provider::get_logger().log_file_content(prefix);
			logger_provider::get_logger().log_file_content(msg);
			logger_provider::get_logger().log_file_content("\n");
		}
	}
	...
	*/
	...
	{
		logger_provider::get_logger().log_file_content("\n");
		logger_provider::get_logger().log_file_content(path_);
		logger_provider::get_logger().log_file_content(" ");
		logger_provider::get_logger().log_file_content("\n");
	}
	...
	*/
	...
	inline void ProcessCmdLine::showOptions()
	{
		for (auto opt : options_)
		{
			logger_provider::get_logger().log_file_content("\n");
			logger_provider::get_logger().log_file_content("/");
			logger_provider::get_logger().log_file_content(char(opt));
			logger_provider::get_logger().log_file_content(" ");
			logger_provider::get_logger().log_file_content("\n");
		}
	}
	...
	*/
	...
	inline ProcessCmdLine::Patterns ProcessCmdLine::patterns() const
	{
		return patterns_;
	}
	...
	inline void ProcessCmdLine::showPatterns()
	{
		...
		{
			logger_provider::get_logger().log_file_content("\n");
			logger_provider::get_logger().log_file_content(patt);
			logger_provider::get_logger().log_file_content(" ");
			logger_provider::get_logger().log_file_content("\n");
		}
	}
	...
	*/
	...
	inline ProcessCmdLine::Number ProcessCmdLine::maxItems() const
	{
		return maxItems_;
	}
	...
	inline void ProcessCmdLine::showMaxItems()
	{
		...
		{
			logger_provider::get_logger().log_file_content("\n");
			logger_provider::get_logger().log_file_content(maxItems_);
			logger_provider::get_logger().log_file_content(" ");
			logger_provider::get_logger().log_file_content("\n");
		}
	}
	...
	*/
	inline bool ProcessCmdLine::parseError() const
	{
		return parseError_;
	}
	...
	*/
	inline ProcessCmdLine::ProcessCmdLine(int argc, char** argv, std::ostream& out) : out_(out)
	{
		...
		{
			logger_provider::get_logger().log_file_content("\n");
			logger_provider::get_logger().log_file_content("command line parse error");
			logger_provider::get_logger().log_file_content("\n");
			usage();
			parseError_ = true;
			return;
	...
	inline void ProcessCmdLine::showCmdLine(int argc, char** argv)
	{
		for (int i = 0; i < argc; ++i)
		{
			...
			{
				logger_provider::get_logger().log_file_content("/");
				logger_provider::get_logger().log_file_content(char(argv[i][1]));
				logger_provider::get_logger().log_file_content(" ");
			}
			else
			{
				logger_provider::get_logger().log_file_content(argv[i]);
				logger_provider::get_logger().log_file_content(" ");
			}
		}
	}
	...
	inline void ProcessCmdLine::usage() const
	{
	...
	*/
	...
	public:
		...
		std::string toXml(const std::string& tag) override
		{
			std::ostringstream out;
			out << "<" << tag << ">" << *this << "</" << tag << ">";
			return out.str();
