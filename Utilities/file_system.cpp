...
file_system::File::File(const std::string& filespec)
...
bool file_system::File::open(direction dirn, type typ)
{
	dirn_ = dirn;
	typ_ = typ;
	good_ = true;
...
std::string file_system::File::getLine(bool keepNewLines)
{
...
std::string file_system::File::readAll(bool keepNewLines)
{
	std::string store;
	while (true)
	{
...
void file_system::File::putLine(const std::string& s, bool wantReturn)
{
...
Block file_system::File::getBlock(size_t size)
{
	if (pIStream == nullptr || !pIStream->good())
		throw std::runtime_error("input stream not open");
	if (typ_ != binary)
...
void file_system::File::putBlock(const Block& blk)
{
	if (pOStream == nullptr || !pOStream->good())
		throw std::runtime_error("output stream not open");
	if (typ_ != binary)
...
size_t file_system::File::getBuffer(size_t bufLen, File::byte* buffer)
{
	if (pIStream == nullptr || !pIStream->good())
		throw std::runtime_error("input stream not open");
	if (typ_ != binary)
...
void file_system::File::putBuffer(size_t bufLen, File::byte* buffer)
{
	if (pOStream == nullptr || !pOStream->good())
		throw std::runtime_error("output stream not open");
	if (typ_ != binary)
...
bool file_system::File::exists(const std::string& file)
{
	return ::GetFileAttributesA(file.c_str()) != INVALID_FILE_ATTRIBUTES;
}
//----< copy file >----------------------------------------------------

bool file_system::File::copy(const std::string& src, const std::string& dst, bool failIfExists)
{
	return ::CopyFileA(src.c_str(), dst.c_str(), failIfExists) != 0;
}
//----< remove file >--------------------------------------------------

bool file_system::File::remove(const std::string& file)
{
...
std::string file_system::Path::toLower(const std::string& src)
{
	std::string temp;
...
std::string file_system::Path::toUpper(const std::string& src)
{
	std::string temp;
	for (size_t i = 0; i < src.length(); ++i)
		temp += toupper(src[i]);
...
std::string file_system::Path::getName(const std::string &fileSpec, bool withExt)
{
...
std::string file_system::Path::getExt(const std::string& fileSpec)
{
...
std::string file_system::Path::getPath(const std::string &fileSpec)
{
	size_t pos = fileSpec.find_last_of("/");
	if (pos >= fileSpec.length())
		pos = fileSpec.find_last_of("\\");
...
std::string file_system::Path::getFullFileSpec(const std::string &fileSpec)
{
...
std::string file_system::Path::fileSpec(const std::string &path, const std::string &name)
{
	std::string fs;
...
{
	hFindFile = ::FindFirstFileA(file_system::Path::fileSpec(path, pattern).c_str(), pFindFileData);
	if (hFindFile != INVALID_HANDLE_VALUE)
	{
